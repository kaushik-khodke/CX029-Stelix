    ...
    <ActivePill active={active}>
      ...
      >
        ...
        {typeof item.badge === "number" && item.badge > 0 ? (
          <Badge variant="secondary" className="ml-1 bg-teal-500/15 text-teal-600 dark:text-teal-400 font-bold border border-teal-500/30">
            {item.badge}
          </Badge>
        ) : null}
      </Link>
...
export function Navbar() {
  ...
  // Role-appropriate home path used in the dashboard dropdown link
  const dashboardHref =
  ...
  const cmdItems = useMemo(() => {
    const base = [
      ...
      { label: "Health Tracker", to: "/patient/routines", hint: "Agenda & vitals" },
      // hospital
      { label: "Triage Queue", to: "/hospital/triage", hint: "Patient queue" },
      { label: "Load Balancer", to: "/hospital/load-balancer", hint: "Resource load" },
      // doctor
            ...
            <Link to="/" className="flex items-center gap-3 min-w-[180px] group">
              <div className="relative">
                ...
                <motion.div
                  ...
                  className="relative h-10 w-10 rounded-xl bg-gradient-to-br from-blue-600 via-teal-500 to-emerald-500 flex items-center justify-center shadow-lg shadow-primary/25"
                >
              ...
              <div className="leading-tight">
                <div className="font-black font-heading text-xl tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-blue-700 via-teal-500 to-emerald-600 dark:from-blue-400 dark:via-teal-300 dark:to-emerald-400">
                  MyHealth<span className="opacity-80 font-bold text-teal-600 dark:text-teal-400">Chain</span>
                </div>
                <div className="text-[9px] uppercase tracking-[0.2em] font-black text-primary/70 hidden sm:block">
            ...
            <div className="flex items-center gap-2">
              ...
              {user ? (
                ...
                >
                  ...
                  {pendingConsents > 0 ? (
                    <span className="absolute -top-1 -right-1 h-5 min-w-5 px-1 rounded-full bg-red-500 text-white text-[10px] font-bold shadow-sm flex items-center justify-center animate-pulse">
                      {pendingConsents}
                    </span>
                  ) : null}
                </button>
