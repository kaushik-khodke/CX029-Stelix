...
export default function Dashboard() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/doctor/dashboard-data?user_id=${uid}`);
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mb-3">
              {doctorData?.specialization ?? "General Physician"} • License: {doctorData?.license_id ?? "N/A"}
            </p>
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-indigo-500/10 text-indigo-600 border border-indigo-500/20">
                {doctorData?.shift_type || "Morning Shift"}
              </span>
              <span className="px-2.5 py-1 text-xs font-bold rounded-md bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">
                {doctorData?.ward_assigned || "General Ward"}
              </span>
            </div>
          </div>
