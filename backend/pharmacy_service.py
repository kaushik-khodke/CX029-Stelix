...
class PharmacyService:
    ...
    async def get_patient_profile(self, patient_id: str) -> Dict[str, Any]:
        """Returns patient profile, supports internal UUID, external_id, or user_id."""
        try:
            # 1. Try internal UUID (patients.id)
            ...
            # 2. Try auth user_id (connecting auth logic to patients table)
            if is_uuid:
                response = self.supabase.table("patients").select("*").eq("user_id", patient_id).maybe_single().execute()
                if response and getattr(response, 'data', None):
                    return response.data

            # 3. Try external_id (e.g. PAT002)
            response = self.supabase.table("patients").select("*").eq("external_id", patient_id).maybe_single().execute()
            if response and getattr(response, 'data', None):
                return response.data
            
            # 4. Fallback: check raw history to see if it even exists
            ...
            if raw_check.data:
                return {"id": None, "external_id": patient_id, "full_name": f"Patient {patient_id}"}
    ...
    async def get_patient_orders(self, patient_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            # 0. Resolve the identity to get both internal id and external_id
            profile = await self.get_patient_profile(patient_id)
            if not profile:
                return []
            
            internal_id = profile.get("id")
            external_id = profile.get("external_id")
            
            flattened = []
            
            # 1. Fetch from standard tables (using internal UUID)
            if internal_id:
                    ...
                    for order in standard_res.data:
                        for item in order.get("order_items", []):
                            ...
                            flattened.append({
                                ...
                                "medicine_id": med.get("id") if med else None,
                                "medicine_name": med.get("name") if med else "Unknown",
                                "strength": med.get("strength") if med else None,
                                "unit_type": med.get("unit_type") if med else None,
                                "price_rec": med.get("price_rec") if med else 0,
                                "package_size": med.get("package_size") if med else None
                            })

            # 2. Fetch from order_history_raw (using external_id)
            if external_id:
                raw_res = self.supabase.table("order_history_raw")\
                    .select("*")\
                    .eq("patient_external_id", external_id)\
